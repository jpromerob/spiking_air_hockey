...
def trigger_process(args):
    ...
    if args.mode == 'Synthetic':
        os.system("python3 ../generation/puck_generator.py -s 0.1 -d 0.15 -m zigzag")
    else:
        if args.recording == '':
            ...
            cmd += f"output udp {args.ip} {args.port} "
            cmd += f"input file {args.recording}"
            os.system(cmd)
...
def parse_args():
    ...
    parser.add_argument('-i', '--ip', type= str, help="IP for events", default='172.16.222.30')
    ...
    parser.add_argument('-m', '--mode', type=str, help="Mode: Synthetic | Recording", default='Synthetic')
    parser.add_argument('-r', '--recording', type=str, help="Recording path", default='')
