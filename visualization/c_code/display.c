...
#define WINDOW_HEIGHT BOARD_HEIGHT*NB_SLICES
...
int emptyMatrix[WINDOW_HEIGHT][BOARD_WIDTH];
...
int input_raw_mat[WINDOW_HEIGHT][BOARD_WIDTH]; // where incoming raw data is stored
int shared_raw_mat[WINDOW_HEIGHT][BOARD_WIDTH]; // matrix shared between udp receiver (raw) and render|saver
int visual_raw_mat[WINDOW_HEIGHT][BOARD_WIDTH]; // matrix used by render (raw)


pthread_mutex_t cnn_mutex = PTHREAD_MUTEX_INITIALIZER;
int input_cnn_mat[WINDOW_HEIGHT][BOARD_WIDTH]; // where incoming scnn data is stored
int shared_cnn_mat[WINDOW_HEIGHT][BOARD_WIDTH]; // matrix shared between udp receiver (scnn) and render|saver
int visual_cnn_mat[WINDOW_HEIGHT][BOARD_WIDTH]; // matrix used by render (scnn)
int location_cnn_mat[WINDOW_HEIGHT][BOARD_WIDTH]; // matrix used to find XY location

pthread_mutex_t mux_mutex = PTHREAD_MUTEX_INITIALIZER;
int cur_x = BOARD_WIDTH/2;
int cur_y = BOARD_HEIGHT/2;
int mux_color = WHITE;
int mux_id = NB_NETS;


pthread_mutex_t cur_tip_mutex = PTHREAD_MUTEX_INITIALIZER;
int cur_tip_x = BOARD_WIDTH/2;
int cur_tip_y = BOARD_HEIGHT/2;

pthread_mutex_t des_tip_mutex = PTHREAD_MUTEX_INITIALIZER;
int des_tip_x = BOARD_WIDTH/2;
int des_tip_y = BOARD_HEIGHT/2;
        ...
        do {
            ...
            for (int i = 0; i < recv_len; i += 4) {
                ...
                for (int j = 0; j < NB_SLICES; j++) {
                    input_raw_mat[y+BOARD_HEIGHT*j][x] += 1;
                }
            }
...
void* updateCnn(void* arg) {
    ...
    switch (screen_id) {
        case 0:
            port_cnn = VIS_FAST_PORT;
            break;
        case 1:
            port_cnn = VIS_MEDIUM_PORT;
            break;
        case 2:
            port_cnn = VIS_SLOW_PORT;
            break;
        default:
            port_cnn = VIS_RAW_PORT;
            break;
    }
    ...
    while (!local_end) {
        ...
        do {
            ...
            for (int i = 0; i < recv_len; i += 4) {
                ...
                input_cnn_mat[y+BOARD_HEIGHT*screen_id][x] += 1;
            }
...
void* updateShared(void* arg) {
    ...
    while (!local_end) {
        ...
        memcpy(shared_raw_mat, input_raw_mat, sizeof(int) * BOARD_WIDTH * WINDOW_HEIGHT);
        pthread_mutex_unlock(&raw_mutex);

        pthread_mutex_lock(&cnn_mutex);        
        memcpy(shared_cnn_mat, input_cnn_mat, sizeof(int) * BOARD_WIDTH * WINDOW_HEIGHT);
        memcpy(location_cnn_mat, input_cnn_mat, sizeof(int) * BOARD_WIDTH * WINDOW_HEIGHT);
        pthread_mutex_unlock(&cnn_mutex);


        memcpy(input_raw_mat, emptyMatrix, sizeof(int) * BOARD_WIDTH * WINDOW_HEIGHT);
        memcpy(input_cnn_mat, emptyMatrix, sizeof(int) * BOARD_WIDTH * WINDOW_HEIGHT);
...
void* findLocation(void* arg) {
    ...
    while (!local_end) {
        ...
        for (int y = BOARD_HEIGHT*(screen_id+0); y < BOARD_HEIGHT*(screen_id+1); y++) {
...
void* multiplex(void* arg) {
    ...
    receiver_addr.sin_port = htons(CTRL_PUCK_PORT);
    
    if (inet_aton(CPU_IP, &receiver_addr.sin_addr) == 0) {
    ...
    while (!local_end) {
        ...
        memcpy(visual_cnn_mat[BOARD_HEIGHT*(NB_SLICES-1)], shared_cnn_mat[BOARD_HEIGHT*mux_id], sizeof(int) * BOARD_WIDTH * BOARD_HEIGHT);
        ...
        if(mux_id < NB_NETS){
            ...
            local_y_px[NB_SLICES-1] = local_y_px[mux_id]-mux_id*BOARD_HEIGHT+NB_NETS*BOARD_HEIGHT;
        }
        ...
        fraction_x = fraction_x/BOARD_WIDTH;
        fraction_y = ((local_y_px[NB_SLICES-1]-(NB_SLICES-1)*BOARD_HEIGHT));
        fraction_y = fraction_y/BOARD_HEIGHT;
...
void* renderMatrix(void* arg) {
    ...
    SDL_Window* window = SDL_CreateWindow("Air Hockey", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, scale*BOARD_WIDTH, scale*BOARD_HEIGHT, SDL_WINDOW_SHOWN);
    SDL_Renderer* renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
    SDL_Texture* matrixTexture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, BOARD_WIDTH, BOARD_HEIGHT);
    ...
    while (!local_end) {
        ...
        pthread_mutex_lock(&raw_mutex);
        memcpy(visual_raw_mat, shared_raw_mat, sizeof(int) * BOARD_WIDTH * WINDOW_HEIGHT);
        pthread_mutex_unlock(&raw_mutex);

        pthread_mutex_lock(&cnn_mutex);
        memcpy(visual_cnn_mat, shared_cnn_mat, sizeof(int) * BOARD_WIDTH * WINDOW_HEIGHT);
        pthread_mutex_unlock(&cnn_mutex);     
        ...
        for (int y = BOARD_HEIGHT*(screen_id+0); y < BOARD_HEIGHT*(screen_id+1); y++) {
            for (int x = 0; x < BOARD_WIDTH; x++) {
                int index = (y-screen_id*BOARD_HEIGHT) * BOARD_WIDTH + x;
                color = BLACK;
                ...
                if (d_puck >KERNEL_SIZE*4/10-1 && d_puck < KERNEL_SIZE*4/10+1){
                    color = local_mux_color;
                }
                double d_cur_tip = get_distance(x, y-BOARD_HEIGHT*screen_id, local_cur_tip_x, local_cur_tip_y);
                if (d_cur_tip > PADDLE_RADIUS-1 && d_cur_tip < PADDLE_RADIUS+1){
                    color = BLUE;
                }
                double d_des_tip = get_distance(x, y-BOARD_HEIGHT*screen_id, local_des_tip_x, local_des_tip_y);
                if (d_des_tip < 4){
                    color = ORANGE;
                }
...
void* updateCurrentTipPosition(void* arg) {
    ...
    si_me.sin_port = htons(CTRL_PADDLE_CUR_PORT);
    ...
    while (!local_end) {
        ...
        if (recvfrom(s, buf, TIPPOS_BUFF_SIZE, 0, (struct sockaddr *)&si_other, (socklen_t *)&slen) == -1) {
            perror("recvfrom");
            exit(1);
        }
        ...
        pthread_mutex_lock(&cur_tip_mutex);  
        cur_tip_x = (int)(x*BOARD_WIDTH/100);
        cur_tip_y = (int)(y*BOARD_HEIGHT/100);
        pthread_mutex_unlock(&cur_tip_mutex);
...
void* updateDesiredTipPosition(void* arg) {
    ...
    memset((char *)&si_me, 0, sizeof(si_me));
    si_me.sin_family = AF_INET;
    si_me.sin_port = htons(CTRL_PADDLE_CON_PORT);
    si_me.sin_addr.s_addr = htonl(INADDR_ANY);
    ...
    while (!local_end) {
        ...
        pthread_mutex_lock(&des_tip_mutex);  
        des_tip_x = (int)(x*BOARD_WIDTH/100);
        des_tip_y = (int)(y*BOARD_HEIGHT/100);
        pthread_mutex_unlock(&des_tip_mutex);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < NB_NETS; i++) {
        scnn_id[i] = i;
        x_px[i] = BOARD_WIDTH/2;
        y_px[i] = BOARD_HEIGHT*(i+0.5);
    }

    memset(input_raw_mat, 0, sizeof(int) * BOARD_WIDTH * WINDOW_HEIGHT);
    memset(shared_raw_mat, 0, sizeof(int) * BOARD_WIDTH * WINDOW_HEIGHT);
    memset(visual_raw_mat, 0, sizeof(int) * BOARD_WIDTH * WINDOW_HEIGHT);
    memset(emptyMatrix, 0, sizeof(int) * BOARD_WIDTH * WINDOW_HEIGHT);
