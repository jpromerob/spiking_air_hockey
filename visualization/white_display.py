    ...
    with aestream.UDPInput((res_x, res_y), device = 'cpu', port=args.port) as stream1:
        ...
        while True:
            ...
            if counter < acc_nb:
                ...
                frame[0:res_x,0:res_y,0] =  reading
                frame[0:res_x,0:res_y,2] =  reading

                reading = np.zeros((res_x,res_y))
                counter = 0

                image = 1-10*cv2.resize(frame.transpose(1,0,2), (new_l, new_w), interpolation = cv2.INTER_AREA)/4
