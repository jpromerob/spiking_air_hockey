...
try:
    ...
    def profile(func):
        return func  # Dummy decorator for normal execution
...
class ControlNet(torch.nn.Module):
    def __init__(self, in_x_sz, in_y_sz, mode): #x size, y size (from robot's perspective)
        ...
        # Set Actual Weights
        self.f_conv.weight.data = self.f_kernel
        self.m_conv.weight.data = self.m_kernel
        self.s_conv.weight.data = self.s_kernel
    ...
    def forward(self, cnn_input):
        ...
        tracking_out = torch.zeros((1, self.input_x_sz, self.input_y_sz), device=f_conv_out.device)
...
def send_tracking_data(sock, avg_x, avg_y):
    ...
    for i in range(radius):
        for j in range(radius):
            ...
            data += pack("<I", packed)
    random_port = random.choice([DEFAULT_VIS_F_SCNN_PORT, DEFAULT_VIS_M_SCNN_PORT, DEFAULT_VIS_S_SCNN_PORT])
    sock.sendto(data, (UDP_IP, random_port))
    sock.sendto(data, (UDP_IP, DEFAULT_VIS_POST_PROJECTOR_PORT))


def parse_args():

    parser = argparse.ArgumentParser(description='Neural Algorithms on GPU')

    max_x, max_y, k_size = get_max_res_and_k_size()

    parser.add_argument('-p', '--port', type= int, help="Port for events", default=3333)
    ...
    parser.add_argument('-m', '--mode', type=str, help="Mode ('game' vs 'test')", default="test")
    parser.add_argument('-r','--profiler', action='store_true', help='Profiler Activated')
...
def main():
    ...
    print(f'Creating Model for {args.length}x{args.width} input')
            ...
            while True:
                ...
                try:
                    if len(idx_x[0])>0 and len(idx_y[0]) > 0:
                        ...
                        sock.sendto(data, (UDP_IP, DEFAULT_CTRL_PADDLE_DESIRED_PORT))
                except:
                    pass
