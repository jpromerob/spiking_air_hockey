...
def add_dims_to_image(dim):
    import matplotlib.image as mpimg

    # Load the image
    image = mpimg.imread("images/AirHockeyBoardEmpty.png")
    ...
    text = f'{dim.il}' # Inner Length
    x, y = 1600, 320
    ax.text(x, y, text, color="k", fontsize=f_size, ha="center", va="center")

    text = f'{dim.iw}' # Inner width
    x, y = 520, 1120
    ax.text(x, y, text, color="k", fontsize=f_size, ha="center", va="center")

    text = f'{dim.d2ey}' # Margin Y (camera)
    x, y = 2640, 1540
    ax.text(x, y, text, color="k", fontsize=f_size, ha="center", va="center")

    text = f'{dim.d2ex}' # Margin X (camera)
    x, y = 2280, 1860
    ax.text(x, y, text, color="k", fontsize=f_size, ha="center", va="center")
    ...
    text = f'{dim.fl}x{dim.fw}'
    x, y = 600, 1860
    ax.text(x, y, text, color="k", fontsize=f_size*2, ha="center", va="center")

    # Remove axis ticks and labels
    ax.axis("off")
    plt.savefig("images/AirHockeyBoardMarked.png", bbox_inches="tight", pad_inches=0)

def send_dimensions(username, password):

    pipeline_cfg = load_config('../configuration/pipeline.yaml')
    GPU_GATEWAY_IP = pipeline_cfg['SNN_Accelerator']['GPU']['IP']
    SPINNAKER_GATEWAY_IP = pipeline_cfg['SNN_Accelerator']['SpiNNaker']['IP']
    ...
    remote_file_path = input('Path to destination on Remote PC: ')+'/common/homdim.pkl'  # Path to the destination on Computer B
    ...
    for hostname in [GPU_GATEWAY_IP, SPINNAKER_GATEWAY_IP]:
...
def parse_args():
    ...
    parser.add_argument('-fx', '--final-x', type=int, help="Final x resolution after homography", default=256)
    parser.add_argument('-ct', '--camera-type', type=str, help="inivation/prophesee", default="prophesee")
    parser.add_argument('-pwd', '--password', type=str, help="Password", default="")
    parser.add_argument('-usr', '--username', type=str, help="User Name", default="")
...
if __name__ == '__main__':
    ...
    if args.username == "" or args.password == "":
        args.username = input("Type username: ")
        args.password = input("Type password: ")
    ...
    dim = get_dimensions(args.res_x, args.res_y, args.final_x)
    add_dims_to_image(dim)
    print('Make sure Homography works fine')
    ...
    while(True):
        ...
        if shall_calibrate(args):       
            ...
            send_dimensions(args.username, args.password)
