...
if __name__ == "__main__":
    ...
    for file in files:
        for run_counter in range(nb_runs_per_file):
            f_version_name = file[:-7]+f'_v{run_counter+1}.aedat4'
            print(f_version_name)
            os.system(f"python3 analyzer.py -do rec -n {nb_pts}  -f ../recordings/{f_version_name}")
            time.sleep(2)

            os.system(f"python3 analyzer.py -do rec -n {nb_pts}  -f ../recordings/{f_version_name} -g")
            time.sleep(2)
