...
DEFAULT_VIS_POST_PROJECTOR_PORT = int(pipeline_cfg['Visualizers']['Post_Projector']['Port'])
PORT_UDP_INTIME_DATA = pipeline_cfg['Analyzer']['In_Time_Data']['Port']  
PORT_UDP_DELAYED_DATA = pipeline_cfg['Analyzer']['Delayed_Data']['Port'] 
...
def analyze_speed(fname, delayed_coordinates, window_size):
    ...
    on_x = delayed_coordinates[20:-20,1]
    on_y = delayed_coordinates[20:-20,0]
    ...
    if PLOT_CURVES:
        ...
        axs[2].set_ylabel('Speed [px/ms]')

        # Adjust layout to prevent overlapping
        plt.tight_layout()
        plt.savefig(f'results/images/runs/{fname}_speed_profiles.png', format='png', dpi=HIGH_DPI)
    ...
    # Compute histogram
    hist, bins = np.histogram(full_speed, bins=10)
    ...
    if PLOT_CURVES:
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f'results/images/runs/{fname}_speed_histogram.png', format='png', dpi=HIGH_DPI)
        plt.clf()
...
def delayed_process(shared_data):
    ...
    if not shared_data['gpu']:
        ...
        with aestream.UDPInput((dim_fl, 2), device = 'cpu', port=PORT_UDP_DELAYED_DATA) as stream1:
        ...
        sock.bind((DEFAULT_CPU_IP, PORT_UDP_DELAYED_DATA))
...
def evaluate_latency(fname, delayed_x, delayed_y, intime_x, intime_y, t, plot_flag):
    ...
    if PLOT_CURVES and plot_flag:
        ...
        axs[0].scatter(np.arange(len(new_intime_x)), new_intime_x, label='Ground Truth', s=MARKER_SIZE)
        axs[0].scatter(np.arange(len(new_delayed_x)), new_delayed_x, label='Tracking Output', s=MARKER_SIZE)
        ...
        # Subplot 2
        axs[1].scatter(np.arange(len(new_intime_y)), new_intime_y, label='Ground Truth', s=MARKER_SIZE)
        axs[1].scatter(np.arange(len(new_delayed_y)), new_delayed_y, label='Tracking Output', s=MARKER_SIZE)
        ...
        if t == 0:
            ...
            shift_label = "best"
        plt.savefig(f'results/images/runs/{fname}_x_y_comparison_{shift_label}.png', format='png', dpi=HIGH_DPI)
        
        if 'spinnaker' in fname:
            accelerator = 'spinnaker'
        else:
            accelerator = 'gpu'
        plt.savefig(f'results/images/runs/z_current_{accelerator}_x_y_comparison_{shift_label}.png', format='png', dpi=HIGH_DPI)
...
def find_latency_and_error(delayed_coordinates, intime_coordinates, nb_shifts, fname):
    ...
    np.savez(f'results/data/{fname}_t_shift_vs_error.npz', t_shift=t_shift, error=error)
...
def write_to_csv(csv_fname, fname, pipeline, latency, error, min_error, max_speed, mean_speed, dlydreps):
    ...
    if dlydreps > MAX_ACCEPTABLE_DLYREPS:
        ...
        quality = "ok"
    csv_fname = 'results/'+csv_fname+f"_summary_{quality}.csv"
...
def from_frames_to_intime_coordinates(frame_array, time_array, delta_t):
    ...
    for i in np.linspace(int(delta_t/2), nb_pts-1, nb_pts - int(delta_t/2)).astype(int):
        ...
        if PLOT_FRAMES:
            plt.imsave(f'results/images/runs/frame_test_input.png', in_frame)
        ...
        if PLOT_FRAMES:
            plt.imsave(f'results/images/runs/frame_test_output.png', out_frame)
        ...
        if PLOT_FRAMES:
            plt.imsave(f'results/images/runs/frame_test_target.png', target_frame)
...
def intime_process(shared_data):
    ...
    # Bind the socket to the receiver's IP and port
    sock.bind((DEFAULT_CPU_IP, PORT_UDP_INTIME_DATA))
...
def aestream_process(shared_data):
    ...
    f_version_name = shared_data['fname']
    filename = pattern.sub('', f_version_name)

    time.sleep(2)
    raw_port = shared_data['port']
    if shared_data['gpu']:
        print("Sending Raw events to GPU")
        ip = shared_data['GPU_IP']
        port = 3333
    else:
        print("Sending Raw events to SpiNNaker")
        ip = shared_data['SPIF_IP']
        port = 3333

    cmd = ""
    cmd += "/opt/aestream/build/src/aestream "
    cmd += "resolution 1280 720 "
    cmd += "undistortion ../calibration/luts/cam_lut_homography_prophesee.csv "
    cmd += f"output udp {shared_data['CPU_IP']} {raw_port} {ip} {port} "
    cmd += f"input file {filename}"
