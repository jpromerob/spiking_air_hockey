...
PORT_UDP_PADDLE_CONSOLIDATED = pipeline_cfg['Controller']['Paddle_consolidated']['Port']
PORT_UDP_PADDLE_CURRENT = pipeline_cfg['Controller']['Paddle_current']['Port']
PORT_UDP_PADDLE_DESIRED = pipeline_cfg['Controller']['Paddle_desired']['Port'] 
PORT_UDP_PUCK_CURRENT = pipeline_cfg['Controller']['Puck_current']['Port'] 
...
def receive_paddle_xy(shared_data):
    ...
    if not shared_data['gpu']:
        ...
        with aestream.UDPInput((dim.fl, 2), device = 'cpu', port=PORT_UDP_PADDLE_DESIRED) as stream1:
...
def parse_args():
    ...
    parser.add_argument('-l','--liveplot', action='store_true', help='Live Plot On')
...
def initialize_shared_data():
    ...
    shared_data['liveplot'] = args.liveplot
